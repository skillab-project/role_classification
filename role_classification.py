...
def train_job_emerging_classifier(
    ...
    filename = f"completed_analysis_emerging_{model_type_lower}"
    ...
    X = np.zeros((len(jobs), len(SKILL_INDEX)), dtype=np.int8)
    labels = []

    print("🏗️ Building matrix and labeling jobs...")
    for i, job in enumerate(jobs):
        ...
        # Fill Matrix
        for s_lab in job_skill_labels:
            if s_lab in SKILL_INDEX:
                X[i, SKILL_INDEX.index(s_lab)] = 1
    ...
    def response_generator():
        ...
        probs = model.predict_proba(X)[:, 1] if hasattr(model, "predict_proba") else (1 / (1 + np.exp(-model.decision_function(X))))

        coefs = None
        if mode == "linear":
            coefs = model.coef_.ravel() if hasattr(model, "coef_") else (model.feature_log_prob_[1] - model.feature_log_prob_[0])
        ...
        header = {
            ...
            "descriptive_statistics": {
                ...
                "avg_skills_per_job": round(float(np.mean(np.sum(X, axis=1))), 2),
        ...
        for start in range(0, num_jobs, BATCH_SIZE):
            ...
            X_batch = X[start:end]
            batch_impacts = explainer.shap_values(X_batch, approximate=True, check_additivity=False) if mode == "shap" else (X_batch * coefs)
